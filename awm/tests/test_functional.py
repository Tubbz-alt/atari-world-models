...
def test_functional_car_racing():
    ...
    # must exist
    build(GatherObservationsPooled)(number_of_plays=1, steps_per_play=100)
    dir_contents = os.listdir(observations_directory / game)
    # 1 play -> 1 worker
    assert len(dir_contents) == 1
    # 100 steps -> 100 observations
    assert len(os.listdir(observations_directory / game / dir_contents[0])) == 100

    # Train the VAE - after training a state file must exist
    assert not (models_directory / game / "vae.torch").is_file()
    build(TrainVAE)(number_of_epochs=2)
    assert (models_directory / game / "vae.torch").is_file()

    build(PrecomputeZValues)()

    # Train the MDN_RNN - after training a state file must exist
    assert not (models_directory / game / "mdn_rnn.torch").is_file()
    build(TrainMDNRNN)(number_of_epochs=2)
    assert (models_directory / game / "mdn_rnn.torch").is_file()
    ...
    assert not (models_directory / game / "controller-1.torch").is_file()
    build(TrainController)(step_limit=5, reward_threshold=2000)
    assert (models_directory / game / "controller-1.torch").is_file()
